...
当插件的能力或平台支持状态发生变化时，必须同步更新文档和元信息：

- 更新插件 JSON 中的 `supportedPlatforms`、`supportsTaskbarDisplay` 等字段。
- 更新 `README.md` 的插件状态表。
- 更新 `docs/TASK_PLAN.md` 或相关开发文档中的当前状态和后续计划。
- 不要让代码实现、插件元信息和文档长期描述不一致。
